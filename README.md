...
```clj
no.cjohansen/powerpack {:mvn/version "2025.09.16"}
```

With Leiningen:

```clj
[no.cjohansen/powerpack "2025.09.16"]
```
...
Fix buggy live reload on Linux ([David Wagner](https://github.com/wagdav)).
...
Fix bug where Protocol would complain that protocol-relative script URLs didn't
exist.
...
- Improve Powerpack's string parser, avoid parsing strings containing quoted
  text with `read-string`.
