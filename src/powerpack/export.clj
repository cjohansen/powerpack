...
(defn export
  ...
  - `:link-ok?` A function to verify links beyond what Powerpack can do. Use
     this if you have valid internal links outside of the pages that Powerpack
     knows about, e.g. if your site is behind a web server with rewrite rules,
     you proxy other services over the same host, etc. The function receives
     three arguments: the powerpack instance, a map of collected page data
     (available page URLs, asset URLs, etc), and a link map `{:href :url :id :text}`."
  [app-options & [opt]]
